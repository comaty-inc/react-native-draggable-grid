...
var DraggableGrid = function (props) {
    ...
    var itemList = items.map(function (item, itemIndex) {
        return (<block_1.Block onPress={function () {
                onBlockPress.bind(null, itemIndex)();
                if (item.itemData.onPress) {
                    item.itemData.onPress();
                }
            }} onLongPress={setActiveBlock.bind(null, itemIndex, item.itemData)} panHandlers={panResponder.panHandlers} style={getBlockStyle(itemIndex)} dragStartAnimationStyle={getDragStartAnimation(itemIndex)} delayLongPress={props.delayLongPress || 300} key={item.key}>
